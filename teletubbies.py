...
def plot_histogram(df, column):
    ...
    st.write(
        f"This histogram shows the distribution of values for the selected column."
    )
...
def plot_boxplot(df, column):
    ...
    st.write(
        f"This box plot visualizes the spread and outliers of the selected column."
    )
...
def plot_exam_score_by_parental_education(df):
    ...
    fig = px.violin(
        df, x="Parental_Education_Level", y="Exam_Score", box=True, points="all"
    )
    st.plotly_chart(fig)
...
selected_nav = option_menu(
    None,
    ["Introduction", "Visualizations", "Conclusion"],
    icons=["house", "graph-up", "three-dots"],
    menu_icon="cast",
    default_index=0,
...
if selected_nav == "Introduction":
    ...
    st.write(
        "This dataset offers an extensive summary of different elements impacting student exam performance. It encompasses details on study routines, attendance, parental engagement, and other factors contributing to academic excellence."
    )
    ...
    histogram, boxplot, correlation_matrix = st.tabs(
        ["Histogram", "Boxplot", "Correlation Matrix"]
    )
